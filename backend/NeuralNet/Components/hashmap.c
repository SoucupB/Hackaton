                            ...
                            void *udata)
{
    ...
    struct hashmap *map = malloc(sizeof(struct hashmap)+bucketsz);
    if (!map) {
        return NULL;
    }
...
int pairComparer(const void *a, const void *b, void *udata) {
  Pair ap = (Pair)a;
  Pair bp = (Pair)b;
  return ap->first - bp->first;
}

uint64_t pairHasher(const void *item, uint64_t seed0, uint64_t seed1) {
  Pair pair = (Pair)item;
  return hashmap_sip(&pair->first, sizeof(pair->first), seed0, seed1);
}

void setPairValue(int64_t key, float value, struct hashmap *map) {
  hashmap_set(map, &(struct Pair_t){ .first = key, .second = value });
}

struct hashmap *initBufferHash(void) {
  return hashmap_new(sizeof(struct Pair_t), 16, SEED_0, SEED_1, pairHasher, pairComparer, NULL);
}

void storeArrayHashMap(struct hashmap *map, void *array, size_t byteSize, float value) {
  setPairValue(hashmap_murmur(array, byteSize, SEED_0, SEED_1), value, map);
}

void saveWeight(struct hashmap *map, int32_t parentID, int32_t childID, float value) {
  assert(map != NULL);
  int32_t buffer[] = {parentID, childID};
  storeArrayHashMap(map, buffer, sizeof(int32_t) * 2, value);
}

float getPairValue(int64_t value, struct hashmap *map) {
  Pair response = hashmap_get(map, &(struct Pair_t){ .first = value });
  if(response) {
...
float retrieveArrayHashMapValue(struct hashmap *map, void *array, size_t byteSize) {
  return getPairValue(hashmap_murmur(array, byteSize, SEED_0, SEED_1), map);
}

float getWeight(struct hashmap *map, int32_t parentID, int32_t childID) {
  assert(map != NULL);
  int32_t buffer[] = {parentID, childID};
  return retrieveArrayHashMapValue(map, buffer, sizeof(int32_t) * 2);
}
